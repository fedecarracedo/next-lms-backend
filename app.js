...
app.get('/:table/obtenerSegunId/:id', (req, res) => {
    con.query(`SELECT * FROM ${req.params.table} WHERE ${req.params.table}_id = ${req.params.id}`, (err, response, fields) => {
...
app.get('/unidad/obtenerUnidadesCurso/:idCurso', (req, res) => {
    try {
        con.query(`SELECT * FROM unidad WHERE unidad_curso = ${req.params.idCurso}`, (err, response, fields) => {
            res.status(200).header('Access-Control-Allow-Origin', '*').send(response)
        })
    } catch (error) {
        res.status(501)
...
app.get('/leccion/obtenerLeccionesUnidad/:idUnidad', (req, res) => {
    try {
        con.query(`SELECT * FROM leccion WHERE leccion_unidad = ${req.params.idUnidad}`, (err, response, fields) => {
            res.status(200).header('Access-Control-Allow-Origin', '*').send(response)
        })
    } catch (error) {
        res.status(501)
